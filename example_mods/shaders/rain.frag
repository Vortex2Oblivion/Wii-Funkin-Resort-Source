...
vec2 scaledTexCoord(){
  float ratio=u_resolution.xu_resolution.y;
  vec2 scale=vec2(1.0,1.0);
  vec2 offset=vec2(0.0,0.0);
  float ratioDelta=ratio-u_textureRatio;
  if(ratioDelta=0.0){
    scale.y=(1.0+ratioDelta);
    offset.y=ratioDelta2.0;
  }else{
    scale.x=(1.0-ratioDelta);
    offset.x=-ratioDelta2.0;
  }
...
void main() {
  ...
  float a=clamp(cur.au_alphaMultiply-u_alphaSubtract, 0.0,1.0);

  vec2 refraction = (vec2(x,y)-0.5)2.0;
  vec2 refractionParallax=parallax(u_parallaxBg-u_parallaxFg);
  vec2 refractionPos = scaledTexCoord()
    + (pixel()refraction(u_minRefraction+(du_refractionDelta)))
    + refractionParallax;
  ...
  if(u_renderShine){
    ...
    float minShine=maxShine0.18;
    vec2 shinePos=vec2(0.5,0.5) + ((1.0512.0)refraction) -(minShine+((maxShine-minShine)d));
    vec4 shine=texture2D(u_textureShine,shinePos);
    tex=blend(tex,shine);
  }

  vec4 fg=vec4(tex.rgbu_brightness,a);

  if(u_renderShadow){
    float borderAlpha = fgColor(0.,0.-(d6.0)).a;
    borderAlpha=borderAlphau_alphaMultiply-(u_alphaSubtract+0.5);
