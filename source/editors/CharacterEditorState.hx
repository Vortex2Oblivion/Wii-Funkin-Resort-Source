...
class CharacterEditorState extends MusicBeatState
{
	...
	var dumbTextsShadow:FlxTypedGroup<FlxText>;
	...
	override function create()
	{
		...
		dumbTextsShadow = new FlxTypedGroup<FlxText>();
		add(dumbTextsShadow);
		dumbTextsShadow.cameras = [camHUD];

		textAnim = new FlxText(375, 16);
		textAnim.setFormat(Paths.font("wii.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		for (i in 0...tipTextArray.length-1)
		{
			...
			tipText.setFormat(Paths.font("wii.ttf"), 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
	...
	function addCharacterUI() {
		...
		var saveCharacterButton:FlxButton = new FlxButton(reloadImage.x - 10, noAntialiasingCheckBox.y + 40, "Save Character", function() {
			saveCharacter();
		});

		trace(saveCharacterButton.x); 
	...
	function addAnimationsUI() {
		...
		animationDropDown = new FlxScrollableDropDownMenu(15, animationInputText.y - 55, FlxScrollableDropDownMenu.makeStrIdLabelArray([''], true), function(pressed:String) {
		...
		ghostDropDown = new FlxScrollableDropDownMenu(animationDropDown.x + 150, animationDropDown.y, FlxScrollableDropDownMenu.makeStrIdLabelArray([''], true), function(pressed:String) {
			var selectedAnimation:Int = Std.parseInt(pressed);
	...
	function addShadowUI() {
		var tab_group = new FlxUI(null, UI_box);
		tab_group.name = "Shadow";

		var saveCharacterShadowButton:FlxButton = new FlxButton(0, 0, "Save Shadow", function() {
			saveCharacterShadow();
		});

		var skewX:FlxUISlider = new FlxUISlider(char.shadow.skew, 'x', 100, 75, -360, 360, 150, null, 5, FlxColor.WHITE, FlxColor.BLACK);
		skewX.nameLabel.text = 'Shadow Skew X';
		tab_group.add(skewX);
		var skewY:FlxUISlider = new FlxUISlider(char.shadow.skew, 'y', 100, 125, -360, 360, 150, null, 5, FlxColor.WHITE, FlxColor.BLACK);
		skewY.nameLabel.text = 'Shadow Skew Y';
		tab_group.add(skewY);
		var angle:FlxUISlider = new FlxUISlider(char.shadow, 'angle', 100, 25, -360, 360, 150, null, 5, FlxColor.WHITE, FlxColor.BLACK);
		angle.nameLabel.text = 'Shadow Angle';
		tab_group.add(angle);

		var flip:FlxUICheckBox = new FlxUICheckBox(80, 0, null, null, "Flip X", 50);
		flip.checked = char.shadow.flipX;
		if(char.shadow.isPlayer) flip.checked = !flip.checked;
		flip.callback = function() {
			char.shadowFlipX = flip.checked;
			char.shadow.originalFlipX = !char.shadow.originalFlipX;
			if(char.shadow.isPlayer) char.shadow.flipX = !char.shadow.flipX;

			ghostChar.shadow.flipX = char.shadow.flipX;
		};
		var rot:FlxButton = new FlxButton(160, 0, "180°", function() { 
			char.shadow.angle = 180;
		});
		var zoom:FlxButton = new FlxButton(240, 0, "1 Zoom", function() { //flxslider moment
			FlxG.camera.zoom = 1;
		});
		tab_group.add(flip);
		tab_group.add(zoom);
		...
		tab_group.add(saveCharacterShadowButton);
	...
	function reloadCharacterImage() {
		...
		if(char.animationsArray != null && char.animationsArray.length > 0) {
			...
			char.shadow.quickAnimAdd('idle', 'BF idle dance');
		}
	...
	function genBoyOffsets():Void
	{
		...
		for (anim => offsets in char.animOffsets)
		{
			...
			text.setFormat(Paths.font("wii.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function genBoyOffsetsShadow():Void
		{
			var daLoop:Int = 0;
	
			var i:Int = dumbTextsShadow.members.length-1;
			while(i >= 0) {
				var memb:FlxText = dumbTextsShadow.members[i];
				if(memb != null) {
					memb.kill();
					dumbTextsShadow.remove(memb);
					memb.destroy();
				}
				--i;
			}
			dumbTextsShadow.clear();
	
			for (anim => offsets in char.shadow.animOffsets)
			{
				var text:FlxText = new FlxText(175, 20 + (18 * daLoop), 0, anim + " (shadow): " + offsets, 15);
				text.setFormat(Paths.font("wii.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				text.scrollFactor.set();
				text.borderSize = 1;
				dumbTextsShadow.add(text);
				text.cameras = [camHUD];
	...
	function loadChar(isDad:Bool, blahBlahBlah:Bool = true) {
		...
		if(char.animationsArray[0] != null) {
			char.playAnim(char.animationsArray[0].anim, true);
			char.shadow.playAnim(char.animationsArray[0].anim, true);
		}
		char.debugMode = true;
			...
			if(FileSystem.exists(directory)) {
				for (file in FileSystem.readDirectory(directory)) {
					...
					if (!sys.FileSystem.isDirectory(path) && file.endsWith('.json')) {
						...
						if(!charsLoaded.exists(charToCheck) && !charToCheck.endsWith('-shadow')) {
	...
	override function update(elapsed:Float)
	{
		...
		if(!charDropDown.dropPanel.visible) {
			...
			if(char.animationsArray.length > 0) {
				...
				if (FlxG.keys.justPressed.S || FlxG.keys.justPressed.W || FlxG.keys.justPressed.SPACE)
				{
					...
					char.shadow.playAnim(char.animationsArray[curAnim].anim, true);
					genBoyOffsets();
				}
				if (FlxG.keys.justPressed.T)
				{
					...
					char.shadow.addOffset(char.animationsArray[curAnim].anim, char.animationsArray[curAnim].offsets[0], char.animationsArray[curAnim].offsets[1]);
					...
					ghostChar.shadow.addOffset(char.animationsArray[curAnim].anim, char.animationsArray[curAnim].offsets[0], char.animationsArray[curAnim].offsets[1]);
					genBoyOffsets();
				}
				...
				for (i in 0...controlArray.length) {
					if(!FlxG.keys.pressed.ALT){
						...
						if(controlArray[i]) {
							...
							var negaMult:Int = 1;
							if(i % 2 == 1) negaMult = -1;
							char.shadow.animationsArray[curAnim].offsets[arrayVal] += negaMult * multiplier;
	
							char.shadow.addOffset(char.shadow.animationsArray[curAnim].anim, char.shadow.animationsArray[curAnim].offsets[0], char.shadow.animationsArray[curAnim].offsets[1]);
							ghostChar.shadow.addOffset(char.shadow.animationsArray[curAnim].anim, char.shadow.animationsArray[curAnim].offsets[0], char.shadow.animationsArray[curAnim].offsets[1]);

							char.shadow.playAnim(char.shadow.animationsArray[curAnim].anim, false);
							if(ghostChar.shadow.animation.curAnim != null && char.shadow.animation.curAnim != null && char.shadow.animation.curAnim.name == ghostChar.shadow.animation.curAnim.name) {
								ghostChar.shadow.playAnim(char.shadow.animation.curAnim.name, false);
							}
							genBoyOffsets();
						}
					}
		...
		ghostChar.shadow.setPosition(char.shadow.x, char.shadow.y);
		super.update(elapsed);
	}
	...
	function saveCharacterShadow() {
		var json = {
			"animations": char.shadow.animationsArray,
			"image": char.shadow.imageFile,
			"scale": char.shadow.jsonScale,
			"sing_duration": char.shadow.singDuration,
			"healthicon": char.shadow.healthIcon,

			"position":	char.shadow.positionArray,
			"camera_position": char.shadow.cameraPosition,

			"no_antialiasing": char.shadow.noAntialiasing,
			"healthbar_colors": char.shadow.healthColorArray,
			"flip_x": char.shadow.originalFlipX,
			"skewX": char.shadow.skew.x,
			"skewY": char.shadow.skew.y,
			"angle": char.shadow.angle,
		};
		...
		if (data.length > 0)
		{
			...
			_file.addEventListener(Event.CANCEL, onSaveCancel);
			_file.addEventListener(IOErrorEvent.IO_ERROR, onSaveError);
			_file.save(data, daAnim + "-shadow.json");
		}
	}
